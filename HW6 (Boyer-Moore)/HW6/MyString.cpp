// Cpt S 223 Homework Assignment - Boyer-Moore Algorithm for IndexOf
// Eric Chen 11381898
...
int MyString::IndexOf(const char* str, int startIndex) const
{
	...
	// rightmost character occurrence table for the bad character rule
	int rightmost[256];
	memset(rightmost, -1, sizeof(rightmost));		// initially set all values to -1

	for (int i=0; i<strLength; i++) {				// create rightmost index of occurence table 
		rightmost[str[i]] = i; 
	}
	...
	while (strLength+shift <= thisLength)	
	{
		for (int i= strLength-1; i>=0; i--) 
		{
			if (rightmost[m_str[i+shift]] == -1)	// check the table, -1 means character isn't in str
			{
				shift += i + 1;						// shift past this position
				break;
			}
			else if (str[i] != m_str[i+shift])		// if rightmost character is in str
			{
				shift += i - rightmost[m_str[i+shift]];	// adjust shift to align matching chars  
				break;
			}
			else if (i == 0) {						// if all characters matched
				return shift;						// return the index
			}
		}
	}
	return -1;										// substring could not be found
}
